...
// 飞书 Webhook - 必须在最前面，不经过任何中间件
app.post('/feishu/webhook', express.json(), async (req, res) => {
  console.log('=== Feishu Webhook ===');
  ...
  if (type === 'url_verification') {
    console.log('URL verification, returning challenge:', challenge);
  ...
  res.setHeader('Content-Type', 'application/json');
  res.status(200).send(JSON.stringify({ code: 0, msg: 'success' }));
});

app.get('/feishu/webhook', (req, res) => {
  res.setHeader('Content-Type', 'application/json');
  res.status(200).send(JSON.stringify({ status: 'ok', message: 'Feishu webhook endpoint' }));
});
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: ["'self'", "https://api.hcaptcha.com", "https://hcaptcha.com", "https://*.supabase.co", "https://open.feishu.cn"],
...
// 发送人工客服请求到飞书
async function notifyHumanSupportRequest(sessionId, messages, userEmail, customerInfo) {
  if (!FEISHU_WEBHOOK_URL) {
    console.log('Feishu webhook not configured');
    return false;
  }
  ...
  try {
    ...
    if (FEISHU_WEBHOOK_URL) {
      ...
      try {
        ...
        console.log(`✅ Human chat message sent to Feishu: ${customerNo}`);
      } catch (error) {
...
app.post('/api/chat', apiLimiter, async (req, res) => {
  try {
    const { message, sessionId, userEmail, customerInfo } = req.body;
    ...
    // 更新客户信息（如果提供）
    if (customerInfo && !session.customerInfo) {
    ...
    if (isHumanRequest) {
      // 发送通知到飞书
      await notifyHumanSupportRequest(sid, session.messages, session.userEmail, session.customerInfo);

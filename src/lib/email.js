 ...
 * 发送订阅确认邮件
 */
export async function sendSubscriptionConfirmation(email) {
    if (!resend) {
...
export async function sendNewsletter(subject, articles, previewText) {
    if (!resend) {
 ...
 */
export async function sendNewsletterToSubscribers(subscribers, subject, articles, previewText) {
    if (!resend) {
 ...
 * 发送作者申请提交确认邮件（给用户）
 */
export async function sendApplicationConfirmation(email, name) {
    if (!resend) {
        console.log('Resend not configured, skipping email');
        return false;
    }
 ...
 * 发送新申请通知邮件（给管理员）
 */
export async function sendAdminApplicationNotification(applicantName, applicantEmail, company, expertiseAreas, bio) {
    if (!resend) {
        console.log('Resend not configured, skipping email');
        return false;
    }
 ...
 * 发送审核通过邮件（给用户）
 */
export async function sendApprovalEmail(email, name, username, tempPassword) {
    if (!resend) {
        console.log('Resend not configured, skipping email');
        return false;
    }
 ...
 * 发送文章审核通过邮件（给作者）
 */
export async function sendArticleApprovalEmail(email, authorName, articleTitle, articleId) {
    if (!resend) {
        console.log('Resend not configured, skipping email');
        return false;
    }
 ...
 * 发送文章审核拒绝邮件（给作者）
 */
export async function sendArticleRejectionEmail(email, authorName, articleTitle, reason) {
    if (!resend) {
        console.log('Resend not configured, skipping email');
        return false;
    }

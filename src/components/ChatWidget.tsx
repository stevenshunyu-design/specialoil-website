...
const ChatWidget = () => {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end">
      {/* Chat Window */}
      {isOpen && (
        <div className="mb-4 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-200 animate-slideUp">
          {/* Header */}
          <div className="bg-gradient-to-r from-[#003366] via-[#004080] to-[#003366] text-white p-4 relative overflow-hidden">
            {/* Decorative elements */}
            ...
            <div className="relative flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="relative">
                  <div className="w-11 h-11 bg-gradient-to-br from-[#D4AF37] to-[#B8960C] rounded-full flex items-center justify-center shadow-lg">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className={`w-2 h-2 rounded-full ${chatMode === 'ai' ? 'bg-blue-300' : isWaitingForAgent ? 'bg-yellow-300 animate-pulse' : 'bg-green-400'}`}></span>
          ...
          {/* Messages Area */}
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gradient-to-b from-gray-50 to-white" style={{ minHeight: '320px', maxHeight: '420px' }}>
            {messages.map((msg, index) => (
              ...
              >
                ...
                {msg.role !== 'user' && msg.role !== 'system' && (
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#D4AF37] to-[#B8960C] flex items-center justify-center mr-2 flex-shrink-0 shadow">
                ...
                <div
                  className={`max-w-[75%] px-4 py-2.5 shadow-sm ${
                    msg.role === 'user'
                      ? 'bg-gradient-to-br from-[#003366] to-[#004080] rounded-2xl rounded-br-sm'
                      : msg.role === 'system'
                      ? 'bg-amber-50 text-amber-700 text-sm rounded-2xl border border-amber-200'
                      : 'bg-white text-gray-800 rounded-2xl rounded-bl-sm border border-gray-100'
                  }`}
                >
                  {msg.sender_name && msg.role === 'admin' && (
                    <p className="text-xs text-[#D4AF37] font-semibold mb-1">{msg.sender_name}</p>
                  )}
                  <p className={`text-sm leading-relaxed whitespace-pre-wrap ${msg.role === 'user' ? 'text-white' : ''}`}>{msg.content}</p>
                </div>
                
                {/* Avatar for user */}
                {msg.role === 'user' && (
                  <div className="w-8 h-8 rounded-full bg-gray-300 flex items-center justify-center ml-2 flex-shrink-0 shadow">
            ...
            {isLoading && (
              <div className="flex justify-start animate-fadeIn">
                ...
                <div className="bg-white text-gray-800 px-4 py-3 rounded-2xl rounded-bl-sm shadow-sm border border-gray-100">
                  <div className="flex items-center gap-1.5">
          ...
          {chatMode === 'ai' && messages.length <= 2 && (
            <div className="px-4 py-3 border-t border-gray-100 bg-white">
              <p className="text-xs text-gray-500 mb-2 flex items-center gap-1">
                <i className="fa-solid fa-lightbulb text-[#D4AF37]"></i>
                Quick Questions:
              </p>
              <div className="flex flex-wrap gap-2">
                {QUICK_QUESTIONS.map((q, i) => (
                  <button
                    ...
                    className="text-xs px-3 py-1.5 bg-gray-100 hover:bg-[#003366] hover:text-white rounded-full text-gray-700 transition-all duration-200 hover:shadow-md"
                  >
                    {q}
                  </button>
                ))}
          ...
          {chatMode === 'ai' && (
            <div className="px-4 py-2 border-t border-gray-100 bg-white">
              <button
                ...
                className="w-full py-2.5 bg-gradient-to-r from-[#D4AF37] to-[#B8960C] text-white rounded-xl font-medium text-sm hover:shadow-lg transition-all duration-200 flex items-center justify-center gap-2"
              >
            ...
            <div className="flex gap-2 items-center">
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/50 focus:border-[#D4AF37] transition-all bg-gray-50 focus:bg-white"
              ...
              <button
                ...
                className="w-11 h-11 bg-gradient-to-r from-[#003366] to-[#004080] text-white rounded-xl flex items-center justify-center hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed hover:scale-105 active:scale-95"
      ...
      {/* Unread Badge */}
      {!isOpen && (
        <div className="absolute -top-1 -right-1 w-5 h-5 bg-gradient-to-r from-[#D4AF37] to-[#B8960C] rounded-full flex items-center justify-center shadow-lg animate-bounce">
      ...
      {!isOpen && (
        <div className="absolute w-14 h-14 rounded-full bg-[#003366]/30 animate-ping pointer-events-none"></div>
      )}
    </div>
  );
};

    ...
    socket.on('visitor:message', async (data) => {
        ...
        if (!error && savedMessage) {
            ...
            if (session) {
                ...
                // 使用 API 发送消息（支持话题回复）
                const result = await sendFeishuChatMessage(sessionId, customerNo, visitorName, visitorEmail, visitorPhone, message, rootMessageId // 如果有 root_message_id，则作为话题回复
                );
    ...
    if (event?.message) {
        ...
        if (replyMatch) {
            ...
            const { data: sessions } = await client
                ...
                .select('*')
                .eq('status', 'active');
            const targetSession = sessions?.find((s) => s.id.startsWith(sessionShortId));
            if (targetSession) {
...
app.get('/api/images', async (req, res) => {
    try {
        ...
        // 为每个图片生成访问URL
        const images = await Promise.all((result.keys || []).map(async (key) => {
...
function parseTrafficSource(referrer, utmSource, utmMedium) {
    ...
    try {
        ...
        if (socialMedia.some(sm => domain.includes(sm)))
            return 'social';
        // 邮件
        if (domain.includes('mail') || utmMedium === 'email')
            return 'email';
        return 'referral';
    }
    catch {
...
// ==================== 作者系统 API ====================
// 导入邮件发送函数
import { sendVerificationCode, sendApplicationConfirmation, sendAdminApplicationNotification, sendApprovalEmail, sendRejectionEmail, sendSubscriptionConfirmation, sendArticleApprovalEmail, sendArticleRejectionEmail } from './src/lib/email';
// 密码哈希工具
...
app.post('/api/author/apply', async (req, res) => {
    try {
        const { name, email, phone, company, expertiseAreas, bio, verificationCode } = req.body;
        // 验证必填字段
        ...
        // 发送通知邮件给管理员
        await sendAdminApplicationNotification(name, email, company || '', expertiseAreas || [], bio || '');
        res.json({
            success: true,
...
app.get('/api/admin/dashboard', async (req, res) => {
    try {
        ...
        // 并行获取所有统计数据
        const [articlesResult, authorsResult, subscribersResult, visitorsResult, pendingArticlesResult, pendingApplicationsResult] = await Promise.all([
            // 文章统计
...
app.post('/api/admin/articles/:id/review', async (req, res) => {
    try {
        ...
        if (author?.email) {
            if (action === 'approve') {
                await sendArticleApprovalEmail(author.email, author.display_name || author.name || 'Author', article.title, article.id);
            }
            else {
                await sendArticleRejectionEmail(author.email, author.display_name || author.name || 'Author', article.title, rejectionReason);
            }
        }
        res.json({
            success: true,
...
app.put('/api/author/:id/profile', async (req, res) => {
    try {
        ...
        if (expertise_areas)
            updates.expertise_areas = expertise_areas;
        // 更新作者信息
